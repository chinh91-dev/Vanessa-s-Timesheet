...
/**
 * Check if today (in AEST) is the last weekday of the month.
 */
function isLastWeekdayOfMonth(): boolean {
  ...
  let aestDay = now.getUTCDate();
  let aestMonth = now.getUTCMonth();
  let aestYear = now.getUTCFullYear();
  
  if (aestMinutes >= 24 * 60) {
    const nextDay = new Date(Date.UTC(aestYear, aestMonth, aestDay + 1));
    aestDay = nextDay.getUTCDate();
    aestMonth = nextDay.getUTCMonth();
    aestYear = nextDay.getUTCFullYear();
  }
  
  const lastDayOfMonth = new Date(aestYear, aestMonth + 1, 0).getDate();
  
  let lastWeekday = lastDayOfMonth;
  while (true) {
    const d = new Date(aestYear, aestMonth, lastWeekday);
    const dow = d.getDay();
    if (dow !== 0 && dow !== 6) break;
    lastWeekday--;
  }
  
  return aestDay === lastWeekday;
}

function getLastMonthRange(): { startDate: string; endDate: string; monthYear: string } {
  const now = new Date();
  const firstOfMonth = new Date(now.getFullYear(), now.getMonth(), 1);
  const lastOfMonth = new Date(now.getFullYear(), now.getMonth() + 1, 0);
  
  const startDate = firstOfMonth.toISOString().split("T")[0];
  const endDate = `${lastOfMonth.toISOString().split("T")[0]}T23:59:59`;
  const monthYear = firstOfMonth.toLocaleDateString("en-AU", { month: "long", year: "numeric" });
...
const handler = async (req: Request): Promise<Response> => {
  console.log("send-penny-commission-report function invoked");
  ...
  try {
    ...
    const { startDate, endDate, monthYear } = getLastMonthRange();
    console.log(`Generating commission report for: ${monthYear} (${startDate} to ${endDate})`);

    // ========== FETCH ALL DATA ==========

    // 1. All Penny's deals (ever, for context) and deals in period
    ...
    const periodDeals = allDeals?.filter((d: any) => {
    ...
    // 6. Meetings this month (with contact info for existing client check)
    ...
    csvLines.push(`# Penny Sharp - Commission Report - ${monthYear}`);
    ...
    const html = `
      <div style="font-family:Arial,sans-serif;color:#333;max-width:700px;margin:0 auto;">
        <h1 style="color:#1a1a2e;">Monthly Commission Report — Penny Sharp</h1>
        <p style="color:#666;">Report period: <strong>${monthYear}</strong></p>
        
        <h2 style="color:#16213e;border-bottom:2px solid #0f3460;padding-bottom:6px;">Deal Summary</h2>
        <table style="border-collapse:collapse;width:100%;margin:12px 0;">
          <tr><td style="padding:8px 12px;border:1px solid #ddd;background:#f5f5f5;font-weight:bold;">Total Deals Owned</td><td style="padding:8px 12px;border:1px solid #ddd;text-align:right;">${allOwnedDeals.length}</td></tr>
          <tr><td style="padding:8px 12px;border:1px solid #ddd;">New Deals (this month)</td><td style="padding:8px 12px;border:1px solid #ddd;text-align:right;">${newDeals.length}</td></tr>
          <tr><td style="padding:8px 12px;border:1px solid #ddd;">Deals Won</td><td style="padding:8px 12px;border:1px solid #ddd;text-align:right;">${wonDeals.length}</td></tr>
          <tr><td style="padding:8px 12px;border:1px solid #ddd;">Deals Lost</td><td style="padding:8px 12px;border:1px solid #ddd;text-align:right;">${lostDeals.length}</td></tr>
        ...
        <h2 style="color:#16213e;border-bottom:2px solid #0f3460;padding-bottom:6px;">Meetings (this month)</h2>
        <table style="border-collapse:collapse;width:100%;margin:12px 0;">
        ...
        <p style="margin-top:24px;color:#888;font-size:12px;">This is an automated commission report. Full deal-level detail is in the attached CSV.</p>
      </div>
    `;
    ...
    const { error: emailError } = await resend.emails.send({
      ...
      to: RECIPIENTS,
      subject: `Penny Sharp — Monthly Commission Report — ${monthYear}`,
      html,
      attachments: [
        {
          filename: `penny-sharp-commission-${monthSlug}.csv`,
    ...
    if (emailError) {
      console.error("Error sending commission report email:", emailError);
      throw new Error(`Failed to send email: ${JSON.stringify(emailError)}`);
    }

    console.log(`Successfully sent Penny Sharp commission report for ${monthYear} to ${RECIPIENTS.join(", ")}`);

    return new Response(
      JSON.stringify({
        success: true,
        message: `Commission report sent for ${monthYear}`,
        stats: {
    ...
    console.error("Error in send-penny-commission-report:", error);
    return new Response(
